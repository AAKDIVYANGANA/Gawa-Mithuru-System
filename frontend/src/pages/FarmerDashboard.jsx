...
function HomeSection({ user, setActiveSection }) {
  const [stats, setStats] = useState({ totalCattle: 0, sickCattle: 0, todayMilk: 0 });

  useEffect(() => { fetchStats(); }, []);

  const fetchStats = async () => {
    try {
      const [cattleRes, milkRes] = await Promise.all([
        API.get('/cattle'),
        API.get('/milk')
      ]);
      ...
      setStats({ totalCattle: cattleRes.data.length, sickCattle, todayMilk: todayMilk.toFixed(1) });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
        ...
        <SummaryCard icon="💉" label="ඉදිරි එන්නත" value="-" color="bg-purple-100 text-purple-700" onClick={() => setActiveSection('ai')} />
      </div>
